...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        checkpoint_paths = [args.task+'lr{}_epochs{}_bs{}_checkpoint-best.pth'.format(args.lr,args.epochs,args.batch_size)]
...
def save_head_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [args.task+'lr{}_epochs{}_bs{}_checkpoint-linear_classifier-best.pth'.format(args.lr,args.epochs,args.batch_size)]
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
